...
var d = o("image/webp");
var h = (e2) => {
  e2.proxy = new Proxy(e2.proxy || e2, { get: (t, o2) => {
    let s2;
    return void 0 !== (s2 = t[o2]) ? s2.bind?.(e2) || s2 : t?.params?.[o2];
  } });
};

// src/middleware.js
var withContent = async (request) => {
...
var router = (cb) => {
  const r2 = e();
  r2.all("*", h, withContent);
  cb(r2);
  r2.all("*", () => new Response("Not Found.", { status: 404 }));
  return r2;
};
var server = (cb, scheduledCb) => {
  var o = { fetch: (request, env, ctx) => router(cb).handle(request, env, ctx).then(s).catch(n) }
  if (scheduledCb) {
    o.scheduled = scheduledCb;
  }
  return o;
};
