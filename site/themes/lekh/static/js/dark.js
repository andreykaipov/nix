...
function doWhenMatchMedia(mediaQuery, name, ifMatchesOrAlwaysDo, otherwiseDo = null, whenCanceledDo = null) {
    ...
    let mediaQueryResponder = (event, canceling = false) => {
        if (canceling) {
            GWLog(`Canceling media query â€œ${name}â€`);
            ...
            GWLog(`Media query â€œ${name}â€ triggered (matches: ${matches ? "YES" : "NO"})`);
...
function injectModeSelector() {
    ...
    // We pre-query the relevant elements, so we donâ€™t have to run queryAll on
